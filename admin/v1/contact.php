...
if($input->is_ajax()){
	...
	if(empty($last)){
		$sql = "
			SELECT `chat`.`id_chat` AS `id`, `isi` AS `msg`, `name` AS `name`, `year` AS `year`, IF(`expired` < CURRENT_TIMESTAMP OR `status` <> 'enabled', true, false) AS `isBlocked`
			FROM (
				SELECT `id_chat`, `isi` 
				FROM (
	...
	foreach ($output as $key => $value) {
		...
		$output[$key]['year'] = htmlentities($value['year']);
	}
