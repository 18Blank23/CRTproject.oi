(function ($) {
					...
					if(result.length > 0){
						...
						if(isFirst == true){
							if(result.length == config.limit){
								$(chatBox).find('#contacts').append('<a href="#" class="btn-link" id="loadMoreContact">Load more contacts</a>')
							}
	...
	var chat = {
		...
		newChat: function(num){
			if(num > 0){
				this.clearNewChat();
				var output = $(node.newChat).text(num + " new message");
	...
	var node = {
		...
		search: '<div id="search"><label for=""><i class="fa fa-search" aria-hidden="true"></i></label><input type="text" placeholder="Find Messages" /></div>',
		disabled: '<p class="text-center"><i>Cannot send message.</i></p>'
	}
