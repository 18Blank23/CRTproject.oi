(function ($) {
	...
	var node = {
		formRequest: '<form action="" method="post" accept-charset="utf-8" id="startChatBox"><div class="p-3"><div class="form-group"><input type="text" name="name" class="form-control form-control-sm" placeholder="Full Name" required></div><div class="form-group"><input type="text" name="id_number" class="form-control form-control-sm" placeholder="ID Number" required></div><div class="form-group"><input type="text" name="phone" class="form-control form-control-sm" placeholder="Year and Section" required></div><div class="form-group"><textarea name="message" class="form-control form-control-sm" placeholder="Type Your Message..." rows="3" style="resize: none;"></textarea></div><button type="submit" class="btn btn-primary btn-sm btn-block">Get started</button></div></form>',
		chatBox: '<div class="card-footer p-0"><form action="" method="post" accept-charset="utf-8" id="formMessage"><div class="input-group"><input type="text" name="message" class="form-control" id="inputMessage" placeholder="Type Your Message..." autocomplete="off"><div class="input-group-append"><button type="submit" id="sendMessage" class="btn btn-sm my-1 mr-1"><i class="fa fa-send"></i></button></div></div></form></div>',
		mainChat: '<ul id="main-chat"></ul>',
		loadOlder: '<a href="#" class="btn-link mt-2" id="loadOlder">Load previously</a>',
	...
	function __activateEvent(){
		...
		$(chatBox).on('submit', 'form#startChatBox', function(event){
			...
			swal({
				text: 'Do you want to start a conversation?',
				icon: 'info',
				buttons: true
			}).then((value) => {
				if(value == true){
					...
					$.ajax({
						...
						success: function(result){
							if(result.status){
								...
								swal({
									text: 'Failed to get token: '+result.error,
									icon: 'error'
								});
							}
						},
